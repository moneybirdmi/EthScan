...
contract LPStaking is Ownable {
    ...
    function getRewardPerBlock() public view returns (uint256) {
        return
            NFYToken.balanceOf(rewardPool).div(6500).div(10000).mul(
                dailyReward
            );
    }
    ...
    function pendingRewards(uint256 _NFT) public view returns (uint256) {
        ...
        return
            nft._LPDeposited.mul(_accNfyPerShare).div(1e18).sub(
                nft._rewardDebt
            );
    }
    ...
    function stakeLP(uint256 _amount) public {
        require(
            ...
            "emergency withdraw is on, cannot stake"
        );
        ...
        if (nft._LPDeposited > 0) {
            uint256 _pendingRewards =
                nft._LPDeposited.mul(accNfyPerShare).div(1e18).sub(
                    nft._rewardDebt
                );
    ...
    function claimRewards(uint256 _tokenId) public {
        ...
        uint256 _pendingRewards =
            nft._LPDeposited.mul(accNfyPerShare).div(1e18).sub(nft._rewardDebt);
        ...
        onlyPlatform()
    {
        ...
        NFTDetails[_tokenId]._LPDeposited = NFTDetails[_tokenId]
            ._LPDeposited
            .add(_amount);
    ...
    function turnEmergencyWithdrawOn() public onlyOwner() {
        require(
            emergencyWithdraw == false,
            "emergency withdrawing already allowed"
        );
