...
contract Ownable is Context {
    ...
    function removePlatformAddress(address _platformAddress)
        public
        onlyOwner()
    {
        require(
            platformAddress[_platformAddress] == true,
            "not platform address"
        );
        platformAddress[_platformAddress] = false;
...
// File: contracts/NFYStakingV2.sol

pragma solidity ^0.6.10;

interface INFYStakingNFT {
    function nftTokenId(address _stakeholder)
        external
        view
        returns (uint256 id);
...
contract NFYStakingV2 is Ownable {
    ...
    function getRewardPerBlock() public view returns (uint256) {
        return
            NFYToken.balanceOf(rewardPool).mul(dailyReward).div(6500).div(
                10000
            );
    }
    ...
    function getNFTBalance(uint256 _tokenId)
        public
        view
    ...
    function checkIfNFTInCirculation(uint256 _tokenId)
        public
        view
        returns (bool _inCirculation)
    {
    ...
    function pendingRewards(uint256 _NFT) public view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && totalStaked != 0) {
            ...
            _accNfyPerShare = _accNfyPerShare.add(
                nfyReward.mul(1e18).div(totalStaked)
            );
        }

        return
            nft._NFYDeposited.mul(_accNfyPerShare).div(1e18).sub(
                nft._rewardDebt
            );
    }
    ...
    function getTotalRewards(address _address) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < StakingNFT.balanceOf(_address); i++) {
            uint256 _rewardPerNFT =
                pendingRewards(StakingNFT.tokenOfOwnerByIndex(_address, i));
    ...
    function getTotalBalance(address _address) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < StakingNFT.balanceOf(_address); i++) {
            uint256 _balancePerNFT =
                getNFTBalance(StakingNFT.tokenOfOwnerByIndex(_address, i));
    ...
    function updatePool() public {
        ...
        accNfyPerShare = accNfyPerShare.add(
            nfyReward.mul(1e18).div(totalStaked)
        );
        lastRewardBlock = block.number;
    ...
    function stakeNFY(uint256 _amount) public {
        ...
        require(
            NFYToken.balanceOf(_msgSender()) >= _amount,
            "Do not have enough NFY to stake"
        );
        ...
        if (nft._NFYDeposited > 0) {
            uint256 _pendingRewards =
                nft._NFYDeposited.mul(accNfyPerShare).div(1e18).sub(
                    nft._rewardDebt
                );
    ...
    function addStakeholder(address _stakeholder) private {
        (bool success, bytes memory data) =
            staking.call(
                abi.encodeWithSignature("mint(address)", _stakeholder)
            );
        require(success == true, "Mint call failed");
        NFTDetails[StakingNFT.nftTokenId(_msgSender())]
            ._addressOfMinter = _stakeholder;
        NFTDetails[StakingNFT.nftTokenId(_msgSender())]._inCirculation = true;
    }

    function addStakeholderExternal(address _stakeholder)
        external
        onlyPlatform()
    {
        (bool success, bytes memory data) =
        ...
        require(success == true, "Mint call failed");
        NFTDetails[StakingNFT.nftTokenId(_stakeholder)]
            ._addressOfMinter = _stakeholder;
    ...
    function claimRewards(uint256 _tokenId) public {
        require(
            StakingNFT.ownerOf(_tokenId) == _msgSender(),
            "User is not owner of token"
        );
        require(
    ...
    function claimAllRewards() public {
        ...
        for (uint256 i = 0; i < StakingNFT.balanceOf(_msgSender()); i++) {
            uint256 _currentNFT =
                StakingNFT.tokenOfOwnerByIndex(_msgSender(), i);
    ...
    function unstakeNFY(uint256 _tokenId) public {
        ...
        uint256 userReceives =
            amountStaked.div(100).mul(95).add(_pendingRewards);
    ...
    function unstakeAll() public {
        ...
        while (StakingNFT.balanceOf(_msgSender()) > 0) {
            uint256 _currentNFT =
                StakingNFT.tokenOfOwnerByIndex(_msgSender(), 0);
            unstakeNFY(_currentNFT);
        }
    }

    // Will increment value of staking NFT when trade occurs
    function incrementNFTValue(uint256 _tokenId, uint256 _amount)
        external
        onlyPlatform()
    {
        require(
            checkIfNFTInCirculation(_tokenId) == true,
            "Token not in circulation"
        );
        updatePool();
        ...
        NFTDetails[_tokenId]._NFYDeposited = NFTDetails[_tokenId]
            ._NFYDeposited
            .add(_amount);
    ...
    function decrementNFTValue(uint256 _tokenId, uint256 _amount)
        external
        onlyPlatform()
    {
        require(
